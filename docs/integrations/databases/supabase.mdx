...
Supabase's API requires an access token to authenticate requests. Clerk's JWT templates allow you to generate a new valid Supabase access token for each signed in user, granting them access to your data.

To create one:

1. Open your project in the Clerk Dashboard and navigate to the **[JWT Templates](https://dashboard.clerk.com/last-active?path=jwt-templates)** page in the sidebar.
2. Select the **New template** button, then select **Supabase** from the list of options.
3. Configure your template:
    - The value of the **Name** field will be required when using the template in your code.
    - The **Token lifetime** field determines how many seconds your JWT template's token will be valid for. In general, longer lifetimes can be less secure. Unless you have a reason, use the default 60 seconds.
    - 
