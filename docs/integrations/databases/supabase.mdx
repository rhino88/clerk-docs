...
- Configure an RLS policy with Clerk
...
Clerk can generate [JSON Web Tokens](https://en.wikipedia.org/wiki/JSON_Web_Token) (JWTs) to represent each user signed into your app. You can customize these JWTs with [JWT templates](/docs/backend-requests/making/jwt-templates#jwt-templates), enabling you to add any necessary data from third party services, such as Supabase.

After your Clerk application has been created, go to the Clerk Dashboard and navigate to the **[JWT Templates](https://dashboard.clerk.com/last-active?path=jwt-templates)** page. Click on the **New template** button to create a new template based on Supabase.http://localhost:4003/docs
